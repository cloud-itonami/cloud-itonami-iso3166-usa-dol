(ns dolcompliance.operation
  "OperationActor -- one DOL labor-compliance operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (LaborCompliance-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the DOL Labor-Standards Compliance
  ...
  One graph run = one compliance operation. No unbounded inner loop.
  ...
  governor is clean -- see `dolcompliance.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [dolcompliance.dolcompliancellm :as dolcompliancellm]
            [dolcompliance.governor :as governor]
            [dolcompliance.phase :as phase]
...
(defn- commit-record [request _context proposal]
  {:effect  (:effect proposal)
   :path    [(:subject request) (:track request)]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (dolcompliancellm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (dolcompliancellm/-advise advisor store request)]
            {:proposal p :audit [(dolcompliancellm/trace request p)]})))
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :subject (:subject request) :track (:track request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :subject (:subject request) :track (:track request) :by (:by approval)}]}
            {:disposition :hold
