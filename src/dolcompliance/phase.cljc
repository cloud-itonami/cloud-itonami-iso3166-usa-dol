(ns dolcompliance.phase
  "Phase 0->3 staged rollout for the USA-DOL (Department of Labor)
  compliance actor.

    Phase 0  read-only        -- no writes, still governor-gated.
    Phase 1  assisted-intake  -- engagement intake allowed, every write
                                 needs human approval.
    Phase 2  assisted-assess  -- adds per-track compliance assessment
  ...
  structural fact, not a rollout milestone still to come (mirrors this
  repo's own README: '`:filing/submit` is never in any phase's `:auto`
  set'). Drafting a real DOL labor-compliance package and submitting it
  are the two real-world acts this actor performs; both are always a
  human operator's call. `dolcompliance.governor`'s
...
(def read-ops  #{})
(def write-ops #{:engagement/intake :compliance/assess :filing/draft :filing/submit})
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-assess"  :writes #{:engagement/intake :compliance/assess}                          :auto #{}}
...
(defn verdict->disposition
  "Map a DOL Labor-Standards Compliance Governor verdict to a base
